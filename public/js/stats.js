function runStats(){
    let portfolioStats = {
        ...
        topMovers: {
            today: {
                dollarWinners:[{'value':none},{'value':none},{'value':none},{'value':none},{'value':none}],
                dollarLosers:[{'value':none},{'value':none},{'value':none},{'value':none},{'value':none}],
                percentWinners:[{'value':none},{'value':none},{'value':none},{'value':none},{'value':none}],
                percentLosers:[{'value':none},{'value':none},{'value':none},{'value':none},{'value':none}]
            },
            total : {
                dollarWinners:[{'value':none},{'value':none},{'value':none},{'value':none},{'value':none}],
                dollarLosers:[{'value':none},{'value':none},{'value':none},{'value':none},{'value':none}],
    ...
    let mapPort = assets.map(x => {
        ...
        if (x.todayGrowth > portfolioStats.topMovers.today.percentLosers[0].value) {
            console.log('x todatGain is greter',x.todayGrowth);
            portfolioStats.topMovers.today.percentLosers[0] = {'symbol':x.symbol,'value':x.todayGrowth}
        }
        else if (x.todayGrowth > portfolioStats.topMovers.today.percentLosers[1].value) {
            console.log('checking second level comparison');
            portfolioStats.topMovers.today.percentLosers[1] = {'symbol':x.symbol,'value':x.todayGrowth}
        }
        else if (x.todayGrowth > portfolioStats.topMovers.today.percentLosers[2].value) {
            console.log('comparison3 is '+ x.todayGrowth +' > '+portfolioStats.topMovers.today.percentLosers[2].value);
            portfolioStats.topMovers.today.percentLosers[2] = {'symbol':x.symbol,'value':x.todayGrowth}
        }
        else if (x.todayGrowth > portfolioStats.topMovers.today.percentLosers[3].value) {
            portfolioStats.topMovers.today.percentLosers[3] = {'symbol':x.symbol,'value':x.todayGrowth}
        }
        else if (x.todayGrowth > portfolioStats.topMovers.today.percentLosers[4].value) {
            portfolioStats.topMovers.today.percentLosers[4] = {'symbol':x.symbol,'value':x.todayGrowth}
        }
        console.log('todays percent losers',portfolioStats.topMovers.today.percentLosers);
        
        // percent winners today
        if (x.todayGrowth > portfolioStats.topMovers.today.percentWinners[0].value) {
            console.log('x todatGain is greter',x.todayGrowth);
            portfolioStats.topMovers.today.percentWinners[0] = {'symbol':x.symbol,'value':x.todayGrowth}
        }
        else if (x.todayGrowth > portfolioStats.topMovers.today.percentWinners[1].value) {
            console.log('checking second level comparison');
            portfolioStats.topMovers.today.percentWinners[1] = {'symbol':x.symbol,'value':x.todayGrowth}
        }
        else if (x.todayGrowth > portfolioStats.topMovers.today.percentWinners[2].value) {
            console.log('comparison3 is '+ x.todayGrowth +' > '+portfolioStats.topMovers.today.percentWinners[2].value);
            portfolioStats.topMovers.today.percentWinners[2] = {'symbol':x.symbol,'value':x.todayGrowth}
        }
        else if (x.todayGrowth > portfolioStats.topMovers.today.percentWinners[3].value) {
            portfolioStats.topMovers.today.percentWinners[3] = {'symbol':x.symbol,'value':x.todayGrowth}
        }
        else if (x.todayGrowth > portfolioStats.topMovers.today.percentWinners[4].value) {
            portfolioStats.topMovers.today.percentWinners[4] = {'symbol':x.symbol,'value':x.todayGrowth}
        }
    ...
    for (let i=0;i<portfolioStats.topMovers.today.dollarLosers.length;i++) {
        console.log(`#changeDollarLoser${i+1}`);
    ...
    for (let i=0;i<portfolioStats.topMovers.today.percentWinners.length;i++) {
        ...
        `<div id="changePercentWinner${i+1}"><span class="symbol">${portfolioStats.topMovers.today.percentWinners[i].symbol}:</span><span class="value"> $${portfolioStats.topMovers.today.percentWinners[i].value.toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,')}</span></div>`
    }

    //daily percent losers TODO
    for (let i=0;i<portfolioStats.topMovers.today.percentLosers.length;i++) {
        console.log(`#changePercentLoser${i+1}`);
        document.querySelector(`#changePercentLoser${i+1}`).innerHTML = 
        `<div id="changePercentLoser${i+1}"><span class="symbol">${portfolioStats.topMovers.today.percentLosers[i].symbol}:</span><span class="value"> $${portfolioStats.topMovers.today.percentLosers[i].value.toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,')}</span></div>`
    }

    //total dollar losers
    for (let i=0;i<portfolioStats.topMovers.total.dollarLosers.length;i++) {
        console.log(`#dollarLoser${i+1}`);
